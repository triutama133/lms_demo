...
export default function AdminDashboard() {
  ...
  const saveCourseCats = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/categories/assign-course', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ course_id: selectedCourseForCat.id, category_ids: newCatIds }),
      });

      const data = await res.json();

      if (!data.success) {
        showToastNotification(`Gagal menyimpan kategori: ${data.error}`, 'error');
        return;
      }
